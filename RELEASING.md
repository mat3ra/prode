...
`github:mat3ra/prode#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/prode/releases/download/wip-<short-sha>/prode.tgz
```
...
```bash
npm run mat3ra:install -- prode wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/prode@<release-url>` with
